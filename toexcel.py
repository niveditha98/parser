...
if not path.endswith(".jpg"):
    print("Must use a jpg image to run the program.")
    sys.exit(1)
...
for table in tables:
    ...
    for i in range(len(table_entries)):
        ...
        for j in range(len(row)):
            ...
            fname = out + "table/cell" + str(num_img) + ".png"
            cv.imwrite(fname, entry_roi)

            fname1 = utils.run_textcleaner(fname, num_img)
            text = utils.run_tesseract(fname1, num_img, psm, oem)
...
#Get the required data. Here, name is extracted

p=path.split('.jpg')
pa=p.get(0)

workbook = xlrd.open_workbook('excel/tables.xlsx')
worksheet = workbook.sheet_by_index(0)
i=1
while i>0:
  if (worksheet.cell(i, 1).value != xlrd.empty_cell.value):
    x=[]
    x.append(pa)
    x.append('Name')
    s=worksheet.cell(i,1)
    if('name' in s or 'Name' in s or 'NAME' in s):
      y=worksheet.row_values(i)
      x.extend(y[1:])
